...
def Mask(inputs, seq_len, mode='mul'):
    if seq_len == None: #double check
        ...
        mask = tf.cast(tf.sequence_mask(seq_len), tf.float32) #tensorflow 中张量数据类型转换
...
def Dense(inputs, ouput_size, bias=True, seq_len=None):
    ...
    outputs = tf.reshape(outputs, tf.concat([tf.shape(inputs)[:-1], [ouput_size]], 0))  #ensure that last dimension is equal to out_size
    if seq_len != None:
